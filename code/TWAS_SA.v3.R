setwd("/data2/usr/LiuWW/project_3/part9.twas/TWAS_SA/")
...
## expression data
load("/data2/usr/LiuWW/project_3/part7.downstream_analysis/data/TPM_genes_high_SA_193.RData")

## expression data prepare
### 数据保留三位小数
TPM_genes_high_SA_193 <- round(as.matrix(TPM_genes_high_SA_193), digits = 3)

### split
#### Col-0
TPM_genes_high_SA_Col <- as.matrix(TPM_genes_high_SA_193[,193])

TPM_genes_high_SA_Col_96 <- TPM_genes_high_SA_Col

for (i in c(2:96)) {
  TPM_genes_high_SA_Col_96 <- cbind(TPM_genes_high_SA_Col_96, TPM_genes_high_SA_Col)
}

#### Ecotype
eco <- seq(from = 1, by = 2, length = 96)
TPM_genes_high_SA_eco_96 <- TPM_genes_high_SA_193[,eco]

################################
# 获得生态型名称顺序
################################
Ecotype_name <- str_sub(colnames(TPM_genes_high_SA_eco_96), start = 20, end = -1)

#### F1
F1 <- seq(from = 2, by = 2, length = 96)
TPM_genes_high_SA_F1_96 <- TPM_genes_high_SA_193[,F1]

#### F1_vs_MP
# TPM_genes_high_SA_F1_vs_MP_96 <- TPM_genes_high_SA_F1_96/((TPM_genes_high_SA_eco_96 + TPM_genes_high_SA_Col_96)/2)

#### F1_vs_HP
# TPM_genes_high_SA_HP_96 <- TPM_genes_high_SA_Col_96
# for (i in c(1:21014)) {
#  for (j in c(1:96)) {
#    TPM_genes_high_SA_HP_96[i, j] <- max(TPM_genes_high_SA_eco_96[i,j], TPM_genes_high_SA_Col_96[i,j])
#  }
# }
# TPM_genes_high_SA_F1_vs_HP_96 <- TPM_genes_high_SA_F1_96/TPM_genes_high_SA_HP_96

#### F1_vs_PP
TPM_genes_high_SA_F1_vs_PP_96 <- TPM_genes_high_SA_F1_96/TPM_genes_high_SA_eco_96

#### F1_vs_Col
TPM_genes_high_SA_F1_vs_Col_96 <- TPM_genes_high_SA_F1_96/TPM_genes_high_SA_Col_96

### 修改行名列名，方便绘图
colnames(TPM_genes_high_SA_Col_96) <- Ecotype_name
colnames(TPM_genes_high_SA_eco_96) <- Ecotype_name
colnames(TPM_genes_high_SA_F1_96) <- Ecotype_name
# colnames(TPM_genes_high_SA_F1_vs_MP_96) <- Ecotype_name
# colnames(TPM_genes_high_SA_F1_vs_HP_96) <- Ecotype_name
colnames(TPM_genes_high_SA_F1_vs_PP_96) <- Ecotype_name
colnames(TPM_genes_high_SA_F1_vs_Col_96) <- Ecotype_name

rownames(TPM_genes_high_SA_F1_96) <- paste0(rownames(TPM_genes_high_SA_eco_96), "_F1")
# rownames(TPM_genes_high_SA_F1_vs_MP_96) <- paste0(rownames(TPM_genes_high_SA_eco_96), "_F1_vs_MP")
# rownames(TPM_genes_high_SA_F1_vs_HP_96) <- paste0(rownames(TPM_genes_high_SA_eco_96), "_F1_vs_HP")
rownames(TPM_genes_high_SA_F1_vs_PP_96) <- paste0(rownames(TPM_genes_high_SA_eco_96), "_F1_vs_PP")
rownames(TPM_genes_high_SA_F1_vs_Col_96) <- paste0(rownames(TPM_genes_high_SA_eco_96), "_F1_vs_Col")
rownames(TPM_genes_high_SA_Col_96) <- paste0(rownames(TPM_genes_high_SA_Col_96), "_Col")
rownames(TPM_genes_high_SA_eco_96) <- paste0(rownames(TPM_genes_high_SA_eco_96), "_eco")
############################################################################
...
# 表达表型有5种：
EE <- c("TPM_genes_high_SA_eco_96",
        "TPM_genes_high_SA_F1_96",
        "TPM_genes_high_SA_F1_vs_Col_96",
        "TPM_genes_high_SA_F1_vs_PP_96")
...
load("/data2/usr/LiuWW/project_3/part9.twas/TWAS_SA/vigor.RData")
load("/data2/usr/LiuWW/project_3/part9.twas/TWAS_SA/vigor_genes.RData")
...
column_info <- data.frame(pheno = c("LA_F1vsC", "CN_F1vsC", "CA_F1vsC",
                          ...
                          heterosis = c("LA", "CN", "CA",
                                        "LA", "CN", "CA"),
                          expr = c("SA_F1_vs_C", "SA_F1_vs_C", "SA_F1_vs_C",  
                                   "SA_F1_vs_P", "SA_F1_vs_P", "SA_F1_vs_P"))
...
column_info$expr <- factor(column_info$expr, levels = c("SA_F1_vs_C", "SA_F1_vs_P"))
...
column_ha_t <- HeatmapAnnotation(trait = column_info$heterosis,
                               ...
                               col = list(trait = c("LA" = "#E64B35FF",
                                                        "CN" = "#00A087FF",
                                                        "CA" = "#3C5488FF"),
                                          expr2pheno = c("SA_F1_vs_C" = "#FDE74C",
                                                   "SA_F1_vs_P" = "#5BC0EB")
                               ),
                               gp = gpar(col = "black"),
...
pdf(file = "Fig1_TWAS_SA_vigor_genes_cor_heatmap.pdf", width = 6, height = 8)  
...
pdf(file = "Fig3cd_TWAS_SA_vigor_genes_venn.pdf", width = 8, height = 6) 
upset(example,  
...
## output
write.table(x = gene_func, file = "Table1_TWAS_SA_vigor_genes_description.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
#ego <- simplify(ego)
ego_result <- ego@result %>% dplyr::filter(pvalue < 0.01)

## Save the result for future use
save(ego_result, ego_result, file = "ego_result.RData")
load("/data2/usr/LiuWW/project_3/part9.twas/TWAS_SA/ego_result.RData")
...
emapplot(ego, 
         ...
         line_scale = 0.6,
         pie_scale = 2)
## export the plots to "Fig2_TWAS_SA_vigor_genes_ego_emapplot"
...
load("/data2/usr/LiuWW/project_3/part9.twas/TWAS_SA/gene2term_long.RData")
...
pdf(file = "Fig3_TWAS_SA_known_vigor_genes_cor_heatmap.pdf", width = 15, height = 15)
draw(hh, heatmap_legend_side = "right")
dev.off()

#------------------ plot selected GO term
GO_info_select <- GO_info[rownames(mm), c(2,6,7,8,11,12, 1,4,5)]

# 分割列的信息
column_info2 <- data.frame(term = colnames(GO_info_select), class = c(rep("A",6), rep("B",3)))
...
pdf(file = "Fig3_TWAS_SA_known_vigor_genes_cor_heatmap_selected_GOterm.pdf", width = 15, height = 26)
